...
func (r InexRepo) CreateIncomeItem(ctx context.Context, item domain.IncomeItem, user domain.User) (*domain.IncomeItem, error) {
	rows, err := r.Pool.Query(ctx, sqlQueries.CreateIncomeItem, item.Name, user.ID)
	if err != nil {
		return nil, fmt.Errorf("InexRepo - CreateIncomeItem - r.Pool.Query: %w", err)
	}
	defer rows.Close()
...
func (r InexRepo) UpdateIncomeItem(ctx context.Context, item domain.IncomeItem) (*domain.IncomeItem, error) {
	rows, err := r.Pool.Query(ctx, sqlQueries.UpdateIncomeItem, item.Name, item.ID)
	if err != nil {
		return nil, fmt.Errorf("InexRepo - UpdateIncomeItem - r.Pool.Query: %w", err)
	}
	defer rows.Close()
...
func (r InexRepo) CreateCostItem(ctx context.Context, item domain.CostItem, user domain.User) (*domain.CostItem, error) {
	rows, err := r.Pool.Query(ctx, sqlQueries.CreateCostItem, item.Name, user.ID)
	if err != nil {
		return nil, fmt.Errorf("InexRepo - CreateCostItem - r.Pool.Query: %w", err)
	}
	defer rows.Close()
...
func (r InexRepo) UpdateCostItem(ctx context.Context, item domain.CostItem) (*domain.CostItem, error) {
	rows, err := r.Pool.Query(ctx, sqlQueries.UpdateCostItem, item.Name, item.ID)
	if err != nil {
		return nil, fmt.Errorf("InexRepo - UpdateCostItem - r.Pool.Query: %w", err)
	}
	defer rows.Close()
